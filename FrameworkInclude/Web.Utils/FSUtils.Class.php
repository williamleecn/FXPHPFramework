...
use Web\Bean\UploadFileInfoBean;
...
final class FSUtils
{
    ...
    public static function CheckInfo($info)
    {
        ...
        $InfoBean = self::FormatUploadFileInfo($info);
     ...
     */
    public static function FormatUploadFileInfo($info)
    {
        ...
        $InfoBean->setFielExt(strtolower(pathinfo($InfoBean->getFileName(), PATHINFO_EXTENSION)));
    ...
    /**
     * @param UploadFileInfoBean $InfoBean
     * @return string
     */
    public static function ReadAllData($InfoBean)
    {
        return file_get_contents($InfoBean->getTmpName());
    }
     ...
     * @return string
     */
    public static function GetUploadFileMD5($InfoBean)
    {
        return md5_file($InfoBean->getTmpName());
    }
    ...
    public static function SaveUploadFile($InfoBean, $path, $Rename = true, $DirPerMon = null)
    {
        ...
        if (!empty($DirPerMon)) {
            $path .= '/' . $DirPerMon;
            if (!is_dir($path)) FSUtils::mkdirs($path);
        }
